...
if(f.fit.summary$coefficients[1]<50){
  is_small<-TRUE}

if(f.fit.summary$coefficients[1]>50 & f.fit.summary$coefficients[1]<100){
  is_med<-TRUE}

if(f.fit.summary$coefficients[1]>100){
  is_large<-TRUE}
...
The predicted von Bertalanffy growth curve for NMFS managed fish species. Growth parameters of Linf (Length infinty), K (growth coefficient), and t0 (size at time 0) were estimated using non-linear least square model. The starting point for model building is accomplished using FSA::vbStarts. 
