...
L50.1980<-predict.glm(maturity.mod, data.frame(LENGTH=seq(min(selected.surv.f$LENGTH),max(selected.surv.f$LENGTH)), decade=rep("1980",length(seq(min(selected.surv.f$LENGTH),max(selected.surv.f$LENGTH)))), type="terms"))
selected.surv.LENGTH
t1<-data.frame(L50.1980,selected.surv.LENGTH)

t1 %>% ggplot(aes(x=selected.surv.LENGTH, y=boot::inv.logit(L50.1980)))+geom_point()
