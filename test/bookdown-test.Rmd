...
} else print ("NOT ENOUGH DATA TO FIT A CURVE")
...
if(nrow(selected.surv.clean) > 0){
  ...
  fig <- ggplot(data=selected.surv.clean,
                aes(x=AGE, 
                    y=LENGTH,
                    color = YEAR %>% as.numeric()))+
    geom_jitter(alpha = 0.5)+
    scale_color_gradientn(colors = nmfspalette::nmfs_palette("regional web")(4),
                         name = "Year")+
  ...
  if(nrow(selected.surv.vonb) > 0){
    fig <- fig +
      geom_line(data = selected.surv.vonb,
                inherit.aes = FALSE,
                mapping = aes(x=AGE, 
                              y=LENGTH),
                color= "blue",
