...
The growth may vary through time and between sexes. To assess the potential differences in growth among these groups, the global von Bertalanffy growth model was constructed and the residuals of each observation and the predicted size was tested for significant differences among groups and through time. 
...
if(nrow(selected.surv.w.predicted)>5){
  ...
  selected.surv.w.predicted<- selected.surv.w.predicted %>% filter(SEX %in% c("male", "female"))   
...
The fit of each of these grouping to the global model is visualized as the distribution of standardized residuals for each observation. Groups with good fit to the model should have smaller overall residuals that have equal variance above and below zero. If 


```{r, residuals boxplots, fig.cap = paste(species, "fit to global von b model by sex and decade")}
if(length(ancova.table)>1){
