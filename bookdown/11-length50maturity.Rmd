...
if(nrow(selected_surv_mat_clean>5)){
  ...
  summarise(mat.code.count=n())

mat_sum_season%>% group_by(LENGTH, SEASON)%>%
  mutate(mat.sum=sum(mat.code.count), prop=(mat.code.count/mat.sum)) %>% group_by(LENGTH) %>%
  ggplot(aes(x=LENGTH, y=prop*100, fill=MATURITY))+geom_bar(position="stack", stat="identity")+
...
>>>>>>> 66419c1910e3d1d7248dce3796e696eba57eccb2
