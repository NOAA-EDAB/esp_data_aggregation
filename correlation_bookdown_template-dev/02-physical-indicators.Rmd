...
test <- ecodata::ESP_seasonal_oisst_anom %>%
  ...
  # how to generalize region filtering? 
  # below is just for black sea bass
  dplyr::filter(stringr::str_detect(Pattern_check, "north", negate = TRUE)) %>%
  dplyr::filter(stringr::str_detect(Pattern_check, "south", negate = TRUE)) %>%
  dplyr::distinct() %>%
