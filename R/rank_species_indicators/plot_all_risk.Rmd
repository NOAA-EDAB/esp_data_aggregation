...
data <- read.csv(here::here("data/risk_ranking", "plot_data.csv")) %>%
  ...
  dplyr::mutate(n_stocks = Species %>% unique() %>% length())
guild_data <- read.csv(here::here("data/risk_ranking", "plot_data_guilds.csv"))
...
metric <- c(rep("Most recent value", 2), 
            "Max of all time", 
            rep("Difference between mean of past 10 years and historical mean", 8),
...
```{r, guild_info}
guild_info %>% 
  dplyr::select(Species, Scientific_name, Guild, overall_mean_len) %>%
  dplyr::distinct() %>%
  dplyr::filter(Guild != "") %>%
...
### All risk data
Ranks for all stocks and stock areas. The total risk (sum of all normalized risk values) is shown for each stock.
```{r plot_more, fig.height = 12}
more <- data %>% dplyr::filter(n_stocks > 4) 
...
```{r plkot_less, fig.height = 5}
less <- data %>% 
  dplyr::filter(n_stocks <= 4) %>%
