...
reg <- c(bf_data$Region, recruit_data$Region, asmt$Region) %>% unique()
...
### Past `r nyear` years
Separate geom_gls() functions were fit for each region; trend lines are only shown when the trend was statistically significant, so some plots may have fewer trend lines than regions.
```{r, short} 
if(length(recruit_data$Species) >= 1){
  plot_recruit(recruit_data, 
               n = nyear,
             lin = lines,
             col = colors)
} else print("NO DATA")
...
### Surface temperature
Surface temperature data were pulled from ``r params$surftemp_source``. Separate geom_gls() functions were fit for fall and spring measurements; trend lines are only shown when the trend was statistically significant, so some plots may have fewer than two trend lines. Fall has solid trend lines, and spring has dashed trend lines.
```{r, surftemp}
...
### Bottom temperature
Bottom temperature data were pulled from ``r params$bottemp_source``. Separate geom_gls() functions were fit for fall and spring measurements; trend lines are only shown when the trend was statistically significant, so some plots may have fewer than two trend lines. Fall has solid trend lines, and spring has dashed trend lines.
```{r, bottemp}
...
### Surface salinity
Surface salinity data were pulled from ``r params$surfsalin_source``. Separate geom_gls() functions were fit for fall and spring measurements; trend lines are only shown when the trend was statistically significant, so some plots may have fewer than two trend lines. Fall has solid trend lines, and spring has dashed trend lines.
```{r, surfsalin}
...
### Bottom salinity
Bottom salinity data were pulled from ``r params$botsalin_source``. Separate geom_gls() functions were fit for fall and spring measurements; trend lines are only shown when the trend was statistically significant, so some plots may have fewer than two trend lines. Fall has solid trend lines, and spring has dashed trend lines.
```{r, botsalin}
...
## Biomass
Biomass data were pulled from ``r params$biomass_source``. Separate geom_gls() functions were fit for fall and spring measurements; trend lines are only shown when the trend was statistically significant, so some plots may have fewer than two trend lines. Fall has solid trend lines, and spring has dashed trend lines.
```{r, biomass}
...
### Catch
Catch statistics from ``r params$recruit_source``.
```{r, catch}
...
### CPUE
CPUE statistics from ``r params$recruit_source``.
```{r, CPUE}
if(nrow(asmt_data %>% dplyr::filter(Metric == "CPUE")) > 0){
plot_asmt(asmt_data, 
          metric = "CPUE", 
          ytitle = "CPUE",
          lin = lines, 
...
## Length
Length data were pulled from ``r params$length_source``. Only years with more than 10 fish lengths were considered for analysis. Separate geom_gls() functions were fit for the minimum, mean, and maximum lengths; trend lines are only shown when the trend was statistically significant, so some plots may have fewer than three trend lines.
```{r, length}
