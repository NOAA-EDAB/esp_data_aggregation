...
reg <- c(bf_data$Region, recruit_data$Region) %>% unique()
...
## Length
Length data were pulled from ``r params$length_source``. Separate geom_gls() functions were fit for the minimum, mean, and maximum lengths; trend lines are only shown when the trend was statistically significant, so some plots may have fewer than three trend lines.
```{r, length}
