...
##### Data
```{r, bbmsy_data}
DT::datatable(asmt_sum_data[which(is.na(asmt_sum_data$B.Bmsy) == FALSE), ] %>% 
...
##### Data
```{r, ffmsy_data}
DT::datatable(asmt_sum_data[which(is.na(asmt_sum_data$F.Fmsy) == FALSE), ] %>% 
...
data <- survey_data %>% 
                ...
                dplyr::summarise(Mean_surface_temperature = mean(day_mean_surf) %>%
                                 ...
                                 Mean_bottom_temperature = mean(day_mean_bot) %>%
                                   round(digits = 2)) # mean by season-year
