...
selected_com_sp<-com_landings_clean %>%
  ...
  summarise(total_pounds=sum(Pounds,na.rm = FALSE),total_dollars=sum(Dollars,na.rm = FALSE), price=(total_dollars/total_pounds))


selected_com_sp %>% 
  ggplot2::ggplot(aes(x=Year, y=log(total_dollars)))+
  ggplot2::geom_point()+
