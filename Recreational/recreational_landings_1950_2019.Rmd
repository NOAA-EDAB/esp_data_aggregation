...
title: "Recreational catch of `r stringr::str_to_sentence(params$stock)`  in `r stringr::str_to_sentence(params$state)`"
---
...
```{r data cleaning, message= FALSE, error=FALSE,warning = FALSE}
library("tidyverse")
...
rec_landings_clean %>% filter(common_name==stock_name) %>%
  ggplot2::ggplot(aes(x=Year, y=zoo::rollmean(log(total),7, na.pad=TRUE)))+geom_point()+geom_line()+ggtitle(label = params$stock )+xlab(label= "Year")+ylab(label = " Log transformed  total catch in pounds (lb) ")+theme_minimal()
...
```{r state breakdown, echo= FALSE}
rec.sub<-rec_landings_1950_2019_clean %>%  dplyr::filter(State == params$state & common_name == params$stock)

rec.sub %>% ggplot2::ggplot(aes(x=Year, y= log(Pounds)))+geom_point(size=3)+geom_line(aes(y= zoo::rollmean(log(Pounds),7, na.pad=TRUE)))+ggtitle(params$stock, subtitle = params$state)
