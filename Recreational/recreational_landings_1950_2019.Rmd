...
rec_landings_clean %>% filter(common_name==params$stock) %>%
...
rec_landings_1950_2019_clean %>% filter(common_name==params$stock) %>% group_by(State, Year) %>%
  ggplot2::ggplot(aes(x=Year, y=log(Pounds)))+
  tidyquant::geom_ma(aes(color = State, lty= State),n=5, lwd=1.4,na.rm = TRUE)+
  ggtitle(label = params$stock ,subtitle = "5 year rolling average")+
  xlab(label= "Year")+
  ylab(label = "Log transformed  total catch in pounds (lb) ")+
  theme_minimal()
#figgure out how to display second graph if data is poor 

rec_landings_1950_2019_clean %>% filter(common_name==params$stock) %>% group_by(State, Year) %>%
  ggplot2::ggplot(aes(x=Year, y=log(Pounds)))+
  geom_line(aes(color=State,lty=State),lwd=1.3)+
