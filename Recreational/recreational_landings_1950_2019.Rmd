...
Selected_sp<-rec_landings_1950_2019_clean %>% group_by(Year) %>% filter(!is.na(Pounds) & common_name==params$stock) %>% summarise(total=sum(Pounds,na.rm = FALSE))
selected_by_state<-rec_landings_1950_2019_clean %>% filter(!is.na(Pounds) & common_name==params$stock) %>% group_by(State, Year)
sp_data_qual<-TRUE
st_data_qual<-TRUE
...
## Recreational landings of `r params$stock` across states in the mid-Atlantic and north-east states
