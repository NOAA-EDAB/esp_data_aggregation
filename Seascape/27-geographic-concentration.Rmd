...
Using fisheries independant data from bottom trawls in `survdat`. The unweighted  centroid is one metric that describes the geometric center of the observed range of `r params$species` in a given year. Calculated as the mean of both the latitude and longitude of all tows that captured the species of interest. Changes in this center point can indicate changes in the geograpic range of a species due to a variety of reasons.
...
## Observed density over time
The density of observations for `r params$species` was estimated using Two-dimensional kernel density estimation on a 2d grid as documented in `MASS`. This density estimation is then visualized to indicate areas of greater or lesser probability of occurrence  
...
selected.surv<-selected.surv %>%
  filter(YEAR >1970)%>%
  mutate(decade=ggplot2::cut_width(YEAR, width = 10, center =  1980 ))
