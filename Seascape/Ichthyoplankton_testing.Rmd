...
geo_lim<-selected_spp.1 %>% summarise(latmin= min(lat, na.rm = TRUE )-1,
                                   latmax= max(lat, na.rm = TRUE)+1,
                                   lonmin= min(lon, na.rm = TRUE)-1,
                                   lonmax= max(lon, na.rm = TRUE)+1)
...
selected_spp.1<-filter_all(selected_spp, ~!is.na(.))
 ...
 ggplot(data = ecodata::coast)+
   ...
   geom_point(data =  selected_spp.1 ,aes(x=lon,y=lat,  size=(rel.proportion)))+
   facet_wrap(~(decade))+
   labs(xlab= "Longitude",ylab="Latitude", size="Relative proportion")
