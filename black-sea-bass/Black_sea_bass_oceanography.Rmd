...
S_rec_plot<-S_rec_temp %>% ggplot(aes(x= year,y=log_recruit_devs))+
  ...
  ggtitle(label="",subtitle = "Southern MAB")+
  theme_bw()
...
N_rec_plot<-N_rec_temp %>% ggplot(aes(x= year,y=log_recruit_devs))+
  ...
  xlab("Year")+
  ylab("Log recruitment deviation")+
  ggtitle(label= "",subtitle = "Northern MAB")+
  theme_bw()
...
SMAB_re_join_plot<-SMAB_re_join %>% ggplot(aes(x=mean_bot_T, y=w_bt_temp))+geom_point()+
  xlab(" In situ Mean winter Bottom temp (C)")+
  ylab("Reanalysis Mean winter Bottom temp (C)")+
...
N_cog_95_join<-inner_join(N_cog_95, SHW_NMAB_winter, by= "year")

S_cog_95_join<-inner_join(S_cog_95, SHW_SMAB_winter, by= "year")



N_cog_95_join  %>% with(cor.test(value,mean_SHW     , method=c("pearson")))

s_store<-S_cog_95_join %>%  with(cor.test(value,mean_SHW      , method=c("pearson")))
...
S_eff_area_join<-left_join(S_eff_area, SHW_SMAB_winter, by= "year")

N_eff_area_join  %>% with(cor.test(value,mean_SHW, method=c("pearson")))
S_eff_area_join  %>%  distinct(year, .keep_all=TRUE) %>% with(cor.test(value,mean_SHW, method=c("pearson")))
