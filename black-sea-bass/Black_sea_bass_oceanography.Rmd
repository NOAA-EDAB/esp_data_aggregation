...
```{r cor_testing, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

  # ggplot()+
  # geom_point(aes(x=Year,y= `T`*1000 ), color="red", data=TSanom_MAB)+
  # geom_line(aes(x=Year, y= `T` *1000), data=TSanom_MAB)+
  # geom_point(aes(x=Year,y= ShW.Vol ),color= "green", data=SHW_MAB)+
  # geom_line(aes(x=Year,y= ShW.Vol ), data=SHW_MAB)+
  # ylab("Mean temperature(C)")+
  #   theme(axis.text.x = element_text(angle = 90),
  #       panel.border= element_blank(),
  #       panel.background= element_rect(fill = "white"))+
...
T_shw_MAB<-ccf(x = TSanom_MAB$T,y = SHW_MAB$ShW.Vol.anom)
S_shw_SMAB<-ccf(x = TSanom_SMAB$S,y = SHW_SMAB$ShW.Vol, rm.)
