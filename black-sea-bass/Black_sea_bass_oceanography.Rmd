...
# write.csv(SMAB_winter_bot, file="SMAB_winter_bot.csv")
# write.csv(NMAB_winter_bot, file="NMAB_winter_bot.csv")
# write.csv(MAB_winter_bot, file="MAB_winter_bot.csv")
...
re_join_plot<-ggpubr::ggarrange(SMAB_re_join_plot,NMAB_re_join_plot, ncol = 1, nrow=2)
...
cog_invert_n<-left_join(cog_n, SHW_SMAB_winter, by= "year")
cog_invert_s<-left_join(cog_s, SHW_NMAB_winter, by= "year")

 cog_invert_n %>%  with(cor.test(value,mean_SHW , method=c("pearson")))
 
 cog_invert_s %>%  with(cor.test(value,mean_SHW , method=c("pearson")))
 
cog_invert_n %>% 
  ggplot(aes(x=mean_SHW,y=value))+
  geom_point()+
  xlab("Mean winter Shelf water volume (km^3)")+
  ylab("Center of gravity (Northings)")+
  ggtitle("Center of gravity of northern subunit and southern MAB shelf water volume", subtitle = "Northern subunit")+
  annotate("text",x=1600,y= 4600, label="r=0.33, P-value= 0.012")
...
#### alex vast output and code to extract 
load("C:/Users/ricky.tabandera/Desktop/parameter_estimates.RData")

parameter_estimates$par

results <- plot(parameter_estimates)
...
df_cog %>% filter(season== "SPRING")

# mu+1.6449*SD
#
df_cog_95<-df_cog %>% mutate(upper95=value+(1.6449*sd))


df_cog_95 %>% filter(stock=="North"& variable=="Northings") %>%
  ggplot(aes(x=year))+
  geom_line(aes( y=value))+
  geom_line(aes(y=upper95), color="red")


N_cog_95<-df_cog_95 %>% filter(stock=="North" & variable=="Northings")
S_cog_95<-df_cog_95 %>% filter(stock=="South" & variable=="Northings")

N_cog_95_join<-left_join(N_cog_95, SHW_NMAB_winter, by= "year")

S_cog_95_join<-left_join(S_cog_95, SHW_SMAB_winter, by= "year")


N_cog_95_join  %>% with(cor.test(value,mean_SHW_T_anom     , method=c("pearson")))

S_cog_95_join %>%  with(cor.test(value,mean_SHW      , method=c("pearson")))

N_cog_95_join %>% ggplot()+
  geom_point(aes(x=mean_SHW, y= value))+
  xlab("Winter shelf water volume (km^3)")+
  ylab("Center of gravity upper 95% CI (Northings)")+
  ggtitle("Upper 95% CI Center of gravity and Winter shelf water volume", subtitle = "Northern MAB")+
  annotate("text",x=1500,y= 4450, label="r = 0.099, P-value = 0.629")

S_cog_95_join %>% ggplot()+
  geom_point(aes(x=mean_SHW, y= value))+
  xlab("Winter shelf water volume (km^3)")+
  ylab("Center of gravity upper 95% CI (Northings)")+
  ggtitle("Upper 95% CI Center of gravity and Winter shelf water volume", subtitle = "Southern MAB")+
  annotate("text",x=1500,y= 4450, label="r = -0.359,t = -2.81,df = 53, P-value = 0.007")
...
N_eff_area<-df_effArea %>% filter(size=="North" & survey==1)
S_eff_area<-df_effArea %>% filter(size=="South" & survey==1)
N_eff_area_join<-left_join(N_eff_area, SHW_NMAB_winter, by= "year") 
S_eff_area_join<-left_join(N_eff_area, SHW_SMAB_winter, by= "year")
N_eff_area_join  %>% with(cor.test(value,mean_SHW, method=c("pearson")))
S_eff_area_join  %>% with(cor.test(value,mean_SHW, method=c("pearson")))

N_eff_area_join %>% ggplot()+
geom_point(aes(x=mean_SHW, y= value))+
  xlab("Winter shelf water volume (km^3)")+
  ylab("Effective area occupied ln(Km^2)")+
  ggtitle("Effective area occupied ln(Km^2) and Winter shelf water volume", subtitle = "Northern MAB")+
  annotate("text",x=1750,y= 40000, label="r = -0.333,t = -1.73,df = 24, P-value = 0.096")

S_eff_area_join %>% ggplot()+
geom_point(aes(x=mean_SHW, y= value))+
  xlab("Winter shelf water volume (km^3)")+
  ylab("Effective area occupied ln(Km^2)")+
  ggtitle("Effective area occupied ln(Km^2) and Winter shelf water volume", subtitle = "Southern MAB")+
  annotate("text",x=1750,y= 40000, label="r = -0.467,t = -3.84,df = 24, P-value < 0.001")
