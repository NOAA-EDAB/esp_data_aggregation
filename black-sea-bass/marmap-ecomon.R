...
data <- data.frame(
  ...
  dplyr::filter(abun < 10^36) # high abundance sites are some sort of mistake?
str(data)
...
fig <- ggplot2::ggplot(data = data,
  ...
  ggplot2::labs(title = "Black sea bass larvae observations in MARMAP and ECOMON",
       subtitle = "Presence only; contours and fill.")
...
fig2 <- ggplot2::ggplot(data = data,
  ...
  ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90)) + 
  ggplot2::labs(title = "Black sea bass larvae observations in MARMAP and ECOMON",
       subtitle = "Kernel density estimates based on presence only")
#fig2

tiff(file = here::here("black-sea-bass", "geography2.tiff"),
    # onefile = TRUE,
    width = 11.5,
    height = 16,
    units = "in",
...
tiff(file = here::here("black-sea-bass", "ctenophores.tiff"),
     # onefile = TRUE,
     width = 11.5,
     height = 16,
     units = "in",
