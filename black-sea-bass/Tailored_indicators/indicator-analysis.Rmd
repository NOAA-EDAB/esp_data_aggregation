...
time_rpt <- function(data, col_name = "Val", out_name = "unnamed"){
  ...
  analysis <- data %>%
    dplyr::ungroup() %>%
    dplyr::mutate(long_avg = mean(avg_value) %>%
                    round(digits = 2),
                  long_sd = sd(avg_value) %>%
                    round(digits = 2)) %>%
    dplyr::filter(Time >= 2016) %>%
    dplyr::mutate(yr5_avg = mean(avg_value) %>%
                    round(digits = 2),
                  yr5_sd = sd(avg_value) %>%
                    round(digits = 2),
  ...
  for(i in 1:nrow(analysis)){
    if(analysis$avg_value[i] > (analysis$long_avg[1] + analysis$long_sd[1])) { 
      ...
      status[i] <- "low" 
    } else status[i] <- "neutral"
  }
  
  analysis$avg_value <- paste(analysis$avg_value,
                              status, sep = ", ")
  
  output <- rbind(cbind(analysis$Time, analysis$avg_value),
...
pt_card_time <- dplyr::full_join(rpt_card_time,
