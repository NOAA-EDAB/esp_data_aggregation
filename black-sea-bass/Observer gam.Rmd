...
OB_selected %>% ggplot(aes(x=LON.DD))+geom_histogram()

str(ecodata::coast)

st_crs(ecodata::coast)
...
ggplot()+
  geom_sf(data=OB_geo,shape=21, size=.4)+
...
gam_mod <- gam(HAILWT ~ s(YEAR, by=WGTTYPE)+s(MONTH,bs = "cc",by=WGTTYPE),family= "quasi", data = OB_selected, method = "REML", select=TRUE)
coef(gam_mod)
summary(gam_mod)
