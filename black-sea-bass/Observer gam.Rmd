...
t1<-OB_selected_comp %>%mutate(WEEK=  strftime(DATE, format = "%V"))
...
```{r}
OB_selected_comp %>% ggplot(aes(x=YDAY,y=log(TOT_black_sea_bass_CATCH)))+geom_point()+facet_grid(cols = vars(YEAR))
```
