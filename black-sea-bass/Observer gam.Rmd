...
OB_selected_comp<-OB_selected_comp %>%mutate(WEEK=  strftime(DATE, format = "%V"))

ves_count<-OB_selected_comp %>% group_by(VES_NAME) %>%  tally()

rare.ves<-ves_count %>% filter(n<5)

# remove rare boat from dataset 
OB_selected_comp_rare<-OB_selected_comp %>% filter(!VES_NAME %in% rare.ves$VES_NAME)


```

### Distribution of haul size

Strong peak near zero and long tail indicate a Poisson like distribution. However, the non discrete nature of the data and the extreme tail indidcates a large amount of overdispersion. Procedeing with a negitive binomial distribution to assess the structure of the data 


```{r}
OB_selected_comp %>% ggplot(aes(x=(TOT_black_sea_bass_CATCH )))+
  geom_histogram(binwidth = 100)+
  xlab("Total of BSB catch per haul (lb)")
```
#### Total Hauls by year
The total amount of BSB caught through time appears to increase drasticly, could be due to either increased effort or higher abundance of BSB on the grounds
```{r, hawlwt-by-yr}
...
#### Vessel composition
Large number of vessels in data set with a total of `r length(ves_count$VES_NAME)` unique vessels. May have to remove the rare boats, that occur less than 5 times to reduce the amount of random effects
```{r}
ves_count<-OB_selected_comp %>% group_by(VES_NAME) %>%  tally()

ves_count %>% ggplot(aes(x=VES_NAME, y=n))+geom_bar(stat = "identity")
rare.ves<-ves_count %>% filter(n<5)
...
```{r}
OB_selected_comp %>% ggplot(aes(x=WEEK,y=log(TOT_black_sea_bass_CATCH)))+geom_point()+facet_wrap(~(YEAR), nrow=4, ncol=4)
```
...
gam_mod.g <- gam(TOT_black_sea_bass_CATCH ~ s(YEAR,bs="tp", k=13 )+s(MONTH,bs = "cc",k=12,by=gear_code_obs)+te(start_lon,start_lat, bs="gp", k= 13),family= "nb", data=OB_selected_comp, method = "REML", select=TRUE)

gam_mod.ves <- gam(TOT_black_sea_bass_CATCH ~ VES_NAME+s(YEAR,bs="tp", k=13)+s(MONTH,bs = "cc",k=12)+te(start_lon,start_lat, bs="gp", k= 13),family= "nb", data=OB_selected_comp, method = "REML", select=TRUE)


gam_mod.ds <- gam(TOT_black_sea_bass_CATCH ~ VES_NAME+s(YEAR,bs="tp", k=13)+s(MONTH,bs = "cc",k=12)+te(start_lon,start_lat, bs="ds", k= 13),family= "nb", data=OB_selected_comp, method = "REML", select=TRUE)
...
summary(gam_mod)
qq.gam(gam_mod.p)
plot(gam_mod.g, pages = 2,rug = TRUE,residuals = TRUE,
     pch = 1, cex = 1,shade = TRUE)
plot(gam_mod)
plot(gam_mod.p, scheme = 2)
...
gam.mod.rare<-gam(TOT_black_sea_bass_CATCH ~VES_NAME+s(YEAR,bs="tp", k=13)+s(MONTH,bs = "cc",k=12)+te(start_lon,start_lat, bs="gp", k= 13),family= "nb", data=OB_selected_comp_rare, method = "REML", select=TRUE)
  
  
summary(gam_mod.ves)
summary(gam.mod.rare)
plot(gam.mod.rare)

gam.check(gam.mod.rare)
# summary(gam_mod.ves)
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# R-sq.(adj) =  0.434   Deviance explained = 65.5%
# -REML =  14923  Scale est. = 1         n = 1742
