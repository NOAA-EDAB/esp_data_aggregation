...
abun_f <- get_running_risk(data = abun_survey %>% 
                               dplyr::filter(SEASON == "FALL", Region != "Outside stock area"), 
                             year_source = "YEAR", 
                             ...
                             n_run = 5)
abun_s <- get_running_risk(data = abun_survey %>% 
                               dplyr::filter(SEASON == "SPRING", Region != "Outside stock area"), 
                             year_source = "YEAR", 
                             value_source = "ABUNDANCE", 
                             high = "low_risk",
...
biomass_f <- get_running_risk(data = biomass_surv %>% 
                                  dplyr::filter(SEASON == "FALL", Region != "Outside stock area"), 
                                year_source = "YEAR", 
                                ...
                                n_run = 5)
biomass_s <- get_running_risk(data = biomass_surv %>% 
                                  dplyr::filter(SEASON == "SPRING", Region != "Outside stock area"), 
                                year_source = "YEAR", 
                                value_source = "BIOMASS", 
                                high = "low_risk",
...
recruit <- get_running_risk(data = dat, 
                              year_source = "Year", 
                              value_source = "Value", 
                              high = "low_risk",
...
abun <- get_running_risk(data = dat, 
                           year_source = "Year", 
                           value_source = "Value", 
                           high = "low_risk",
...
biomass <- get_running_risk(data = dat, 
                              year_source = "Year", 
                              value_source = "Value", 
                              high = "low_risk",
...
dat <- asmt %>% dplyr::filter(Metric == "Catch",
                              Units == "Metric Tons")
catch <- get_running_risk(data = dat, 
                          year_source = "Year", 
                          value_source = "Value", 
                          high = "high_risk",
